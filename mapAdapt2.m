function gmm = mapAdapt2(dataList, ubmFilename, tau, config, gmmFilename)
...
% offset 0 values with the slighest bit of noise. unable to inspect n in
% the parfor, but given that the initlization of the variables is to 0,
% sometimes N vector will still have them which breaks rdivide as a x/0!
N(N==0) = eps;

if any(config == 'm'),
    alpha = N ./ (N + tau); % trade-off between ML mean and UBM mean
    % this rdvide returns NaNs when N has zeros!
